...
%Voltar
 ...
 voltarg(casa(X,Y)):- estado(casa(Lin,Col),_,_,_,_,_), Linha is Lin-1, (eh_safe(X), member(casa(Linha,Col),X)),Lin > 1, melhor_acao(andar_baixo), voltar.
 voltarg(casa(X,Y)):- estado(casa(Lin,Col),_,_,_,_,_), Coluna is Col-1, (eh_safe(X), member(casa(Lin,Coluna),X)),Col > 1, melhor_acao(andar_esquerda), voltar.
 voltarg(casa(X,Y)):- estado(casa(Lin,Col),_,_,_,_,_), Coluna is Col+1,(eh_safe(X), member(casa(Lin,Coluna),X)),Col < 12, melhor_acao(andar_direita), voltar.
 voltarg(casa(X,Y)):- estado(casa(Lin,Col),_,_,_,_,_), Linha is Lin+1, (eh_safe(X), member(casa(Linha,Col),X)),Lin <12, melhor_acao(andar_cima), voltar.
		
eh_safenotvisit:- ( adjacente(casa(X,Y),casa(W,Z)), (eh_safe(F), member(casa(X,Y),F)), (eh_safe(F), member(casa(W,Z),F)) , not(percorrido(casa(W,Z))),  W > 0, W< 13, Z >0, Z<13,!), voltarg(casa(X,Y)).
notvisit:- (adjacente(casa(X,Y),casa(W,Z)),(eh_safe(F), member(casa(X,Y),F)), not(percorrido(casa(W,Z))),  W > 0, W< 13, Z >0, Z<13,!), voltarg(casa(X,Y)).

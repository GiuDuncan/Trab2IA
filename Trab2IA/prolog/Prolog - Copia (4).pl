...
eh_safenotvisit:- estado(casa(Lin,Col),_,_,_,_,_),( (eh_safe(F), member(casa(X,Y),F)),adjacente(casa(X,Y),casa(W,Z)),  (eh_safe(F), member(casa(W,Z),F)) , not(percorrido(casa(W,Z))), percorrido(casa(X,Y)),(X \=Lin; Y \=Col),!), voltarg(casa(X,Y)).
notvisit:-  estado(casa(Lin,Col),_,_,_,_,_),((eh_safe(F), member(casa(X,Y),F)),adjacente(casa(X,Y),casa(W,Z)), not(percorrido(casa(W,Z))), percorrido(casa(X,Y)),(X \=Lin; Y \=Col),!), voltarg(casa(X,Y)).
